...
/**
 * The streamer account's Twitch user token (Prisma `TwitchStreamerAuth`). Written by
 * the streamer OAuth flow and by every Twurple refresh; read by
 * `TwitchTokenStoreService.createStreamerAuthProvider()`.
 */
@Entity({ tableName: 'twitch_streamer_auth' })
export class TwitchStreamerAuth {
    ...
    @OneToOne(() => TwitchUser, user => user.registered_streamer_auth, {
        owner: true,
